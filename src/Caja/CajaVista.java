...
public class CajaVista extends JFrame {
    ...
    private JPanel crearKPIs() {
        ...
        lblVentas = crearTarjeta(panel, "VENTAS DEL DÍA", DORADO_HOVER);
        lblEsperado = crearTarjeta(panel, "MONTO ESPERADO", PRECIO);
    ...
    private void cargarKPIs() {
        try (Connection con = ConexionBD.conectar();
            ...
            ResultSet rs = st.executeQuery(
                    "SELECT COALESCE(monto_inicial,0) mi, COALESCE(monto_final,0) mf "
                    + "FROM apertura_caja WHERE estado='Abierta' ORDER BY id DESC LIMIT 1");

            if (rs.next()) {
                double inicial = rs.getDouble("mi");
                double ventas = getVentasDelDia(con);
    ...
    private double getVentasDelDia(Connection con) {
        try (Statement st = con.createStatement()) {
            ResultSet rs = st.executeQuery(
                    "SELECT COALESCE(SUM(total),0) total FROM pedidos WHERE DATE(fecha) = CURDATE()");
    ...
    private double getVentasPorCaja(Connection con, int idApertura, Timestamp desde, Timestamp hasta) {
        ...
        String sql;
        if (hasta != null) {
            sql = "SELECT COALESCE(SUM(total),0) FROM pedidos WHERE id_apertura=? AND fecha BETWEEN ? AND ?";
        } else {
            sql = "SELECT COALESCE(SUM(total),0) FROM pedidos WHERE id_apertura=? AND fecha >= ?";
        }
    ...
    private static CajaVista instancia;

    public static void abrir() {
        if (instancia == null || !instancia.isDisplayable()) {
            instancia = new CajaVista();
        }
