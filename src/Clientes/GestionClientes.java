...
public class GestionClientes extends JFrame {
    ...
    private void initComponents() {
        setTitle("CAFÉ COMETA - GESTIÓN DE CLIENTES");
        setSize(1100, 750);
        ...
        if (confirmacion == JOptionPane.YES_OPTION) {
            try (Connection con = ConexionBD.conectar()) {
                try (PreparedStatement psCheck = con.prepareStatement(
                        "SELECT COUNT(*) FROM pedidos WHERE nombre_cliente = (SELECT nombre FROM clientes WHERE id=?)")) {
                    psCheck.setInt(1, id);
                    ResultSet rsCheck = psCheck.executeQuery();
                    if (rsCheck.next() && rsCheck.getInt(1) > 0) {
                        JOptionPane.showMessageDialog(this, "No se puede eliminar: el cliente tiene pedidos asociados.");
                        return;
                    }
                }
